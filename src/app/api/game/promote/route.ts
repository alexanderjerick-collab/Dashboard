...
export async function POST(req: NextRequest) {
  ...
  if (!permissions?.promote) return NextResponse.json({ error: "Insufficient permissions" }, { status: 403 });
  ...
  try {
    ...
    return NextResponse.json({ success: true, message: "Member promoted" });
  } catch (err) {
